...
export const CorrespondenceForm: React.FC = () => {
  ...
  return (
    <form className="flex flex-col gap-6 p-6 sm:p-8 lg:p-9 rounded-[2rem] bg-white/60 border border-black/5 shadow-brand-shadow" onSubmit={handleSubmit}>
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-muted mb-2">DM control</p>
        <h3 className="text-2xl lg:text-[2rem] font-bold tracking-[-0.04em] text-brand-ink mb-2">Write the message</h3>
        <p className="text-sm text-brand-muted leading-relaxed">Draft your outreach or notes so you stay clean and consistent.</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)] gap-5">
        <label className="flex flex-col gap-1.5">
          <span className="text-[10px] font-bold uppercase tracking-widest text-brand-muted">Where</span>
          <select
      ...
      <label className="flex flex-col gap-1.5">
        <span className="text-[10px] font-bold uppercase tracking-widest text-brand-muted">Message</span>
        <textarea
      ...
      {!opportunityCorrespondence.length && (
        <div className="p-4 rounded-[1.5rem] bg-brand-review/30 border border-brand-review/50 text-sm text-brand-ink">
          <p>No messages yet. Drop recruiter notes or your next DM here.</p>
        </div>
      )}
    </form>
  );
