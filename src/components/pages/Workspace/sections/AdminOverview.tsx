...
export const AdminOverview: React.FC = () => {
  const { state, integrityReport, lastIntegrityRunAt } = useMonyawn();
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <div className="flex flex-col gap-6 p-8 rounded-3xl bg-brand-surface-soft border border-black/5 shadow-brand-shadow">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-widest text-brand-muted">Warnings</span>
              <p className="text-2xl font-bold text-brand-review">{integrityReport.warnings.length}</p>
            </div>
          </div>
          <div className="flex flex-col gap-2 mt-4 max-h-32 overflow-y-auto pr-2">
            {integrityReport.errors.length ? (
              integrityReport.errors.map((item) => (
                <article key={item} className="p-3 rounded-lg bg-red-50 border border-red-100 text-[10px] text-red-600">
            ...
            {integrityReport.warnings.slice(0, 3).map((item) => (
              <article key={item} className="p-3 rounded-lg bg-brand-review/10 border border-brand-review/20 text-[10px] text-brand-ink">
                <p className="font-bold uppercase tracking-tighter mb-1 text-brand-review">Warning</p>
                <p>{item}</p>
              </article>
            ))}
          </div>
      ...
      <div className="col-span-1 md:col-span-2 lg:col-span-3 p-8 rounded-3xl bg-brand-surface-soft border border-black/5 shadow-brand-shadow">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-brand-muted">Approval rate</span>
