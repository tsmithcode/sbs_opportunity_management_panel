# Panel of Experts and Decision Authority

Use this panel for major product and implementation decisions:

1. Founder / CEO
   - brand intent
   - public narrative
   - investor visibility
   - platform thesis

2. CTO / Principal Platform Architect
   - architecture boundaries
   - scalability path
   - technical debt control
   - repo structure

3. CPO / Principal Product Strategist
   - product scope
   - MVP discipline
   - decision-support workflows
   - user outcomes

4. Staff UX Architect
   - user flows
   - progressive disclosure
   - guided interaction design
   - cognitive load reduction

5. Design Systems Lead
   - component consistency
   - tokens
   - spacing
   - hierarchy
   - cross-platform visual language

6. iOS / Mobile Experience Specialist
   - native feel
   - motion
   - gesture behavior
   - App Store readiness implications

7. Microsoft Desktop UX Specialist
   - dense workflows
   - data-heavy interaction patterns
   - keyboard and productivity standards
   - enterprise clarity

8. Accessibility and Compliance Lead
   - WCAG discipline
   - contrast
   - focus order
   - semantics
   - compliance risks

9. Privacy / Governance Architect
   - local-first trust posture
   - data minimization
   - auditability
   - enterprise review posture

10. QA / Release / Evidence Lead
    - regression safety
    - verification
    - proof artifacts
    - release confidence

11. Growth / GTM / Investor Narrative Advisor
    - differentiation
    - demo readiness
    - commercial readability
    - executive storytelling

12. Cultural Strategy / Community Authenticity Lead
   - cultural legitimacy
   - audience trust
   - narrative boundaries
   - authenticity protection
   - community-fit product judgment

13. Trust & Safety / Content Moderation Lead
   - abuse handling
   - harassment controls
   - context-sensitive language policy
   - safety escalation paths
   - misuse prevention

14. Applied Linguist / Sociolinguistics Lead
   - dialect-aware UX
   - vernacular interpretation
   - language-preserving transformation
   - respectful translation logic
   - meaning fidelity

15. Civil Rights / Employment Equity Advisor
   - anti-bias implications
   - fairness review
   - discrimination-sensitive workflows
   - hiring-impact analysis
   - systemic-risk review

16. Community Research / Field Insights Lead
   - lived-experience validation
   - user interviews
   - segment-specific needs
   - founder-assumption checks
   - ground-truth discovery

17. Behavioral Science / Motivation Design Lead
   - habit formation
   - confidence-preserving UX
   - shame avoidance
   - progress loops
   - retention psychology

18. Education / Career Mobility Strategist
   - job-readiness frameworks
   - communication uplift
   - opportunity alignment
   - interview usefulness
   - upward-mobility outcomes

19. Partnerships / Community Distribution Lead
   - community access channels
   - nonprofit/workforce partnerships
   - institutional trust
   - user acquisition in target communities
   - launch partnerships

20. Public Narrative / PR Risk Advisor
   - controversy review
   - founder voice protection
   - press framing
   - backlash planning
   - strategic messaging discipline

21. Data / Experimentation Lead
   - funnel instrumentation
   - retention measurement
   - intervention testing
   - outcome analytics
   - product-learning loops

22. Monetization / Pricing Strategist
   - mission-aligned pricing
   - free vs premium balance
   - access protection
   - sponsor/subsidy models
   - long-term business viability

## Decision Rules
- Use multi-disciplinary reasoning, not single-discipline bias.
- If experts disagree, surface the tradeoff and choose a default.
- Default to the highest-ROI path that preserves current working value.
- Avoid speculative redesign when a targeted enhancement will do.
